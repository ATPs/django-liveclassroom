...
def publish_assessment(
    *, actor, assessment: AssessmentDefinition, expected_version: int, audience: Any = None
    ...
    # ``course`` is optional.  PostgreSQL rejects a plain ``FOR UPDATE`` on
    # the nullable side of the outer join generated by ``select_related``;
    # lock only the draft row while still loading its course for authorization
    # and publication metadata.
    locked = (
        AssessmentDefinition.objects.select_related("course")

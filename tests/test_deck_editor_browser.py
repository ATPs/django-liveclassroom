...
def test_teacher_creates_reorders_and_saves_deck_on_mobile(live_server):
    ...
    try:
        ...
        page.get_by_role("button", name="Preview", exact=True).wait_for()
        page.get_by_label("Title", exact=True).fill("Cell biology")
        page.get_by_role("button", name="Preview", exact=True).click()
        assert page.locator(".lc-deck-preview").is_visible()
        assert not page.locator(".lc-deck-edit-layout").is_visible()
        page.get_by_role("button", name="Edit", exact=True).click()
        assert page.get_by_label("Title", exact=True).input_value() == "Cell biology"

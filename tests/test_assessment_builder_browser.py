...
def test_teacher_builds_reopens_previews_and_copies_fixed_assessment(live_server):
    ...
    try:
        ...
        assert page.evaluate("window.location.search") == "?panel=picker"
        page.locator(".lc-assessment-picker").wait_for()
        biology_bank = page.locator(".lc-question-bank-item").filter(has_text="Biology")
        biology_bank.wait_for()
        biology_bank.click()
        ...
        page.locator(".lc-assessment-outline .lc-assessment-list-item").nth(1).click()
        ...
        page.get_by_role("tab", name="Editor", exact=True).wait_for()
        page.get_by_role("tab", name="Editor", exact=True).click()

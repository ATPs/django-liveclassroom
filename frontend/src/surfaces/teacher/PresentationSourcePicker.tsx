...
export function PresentationSourcePicker({
  ...
  useEffect(() => {
    void Promise.all([
      ...
      refreshCues(),
    ]).catch((error: unknown) => { setStatusTone("error"); setStatus(error instanceof Error ? error.message : tr("Sources are unavailable.", "来源不可用。")); });
  }, [providerUrl, cuesUrl]);
  useEffect(() => {
  ...
  return <section className="lc-console-panel lc-presentation-sources" data-presentation-source-picker>
    <h4>{tr("Presentation sources and cues", "演示来源和提示点")}</h4>
    <div className="lc-presentation-source-grid">
      <label className="lc-field">{tr("Source", "来源")}<select value={mode} onChange={(event) => { setMode(event.target.value as "native" | "external"); setSelectedSource(null); }}><option value="native">{tr("Native deck", "原生幻灯片")}</option><option value="external">{tr("Provider URL", "提供者 URL")}</option></select></label>
      <label className="lc-field">{tr("Lesson step", "教案步骤")}<select value={stepKey} onChange={(event) => setStepKey(event.target.value)}><option value="">{tr("Choose a step", "选择步骤")}</option>{steps.map((step) => <option key={step.key ?? step.id} value={step.key}>{step.position}. {step.title}</option>)}</select></label>
      <label className="lc-field lc-presentation-slide-field">{tr("Slide", "页面")}<input type="number" min={1} value={Number(slideIndex) + 1} onChange={(event) => setSlideIndex(String(Math.max(0, Number(event.target.value) - 1)))} /></label>
    </div>
    {mode === "native" ? <label className="lc-field">{tr("Snapshot", "快照")}<select value={snapshotId} onChange={(event) => setSnapshotId(event.target.value)}><option value="">{tr("Choose a snapshot", "选择快照")}</option>{snapshots.map((snapshot) => <option key={snapshot.id} value={snapshot.id}>{snapshot.title} ({snapshot.slides?.length ?? "?"})</option>)}</select></label> : <>
      <div className="lc-presentation-source-grid">
        <label className="lc-field">{tr("Provider", "提供者")}<select value={provider} onChange={(event) => { setProvider(event.target.value); setResults([]); }}><option value="">{tr("Choose a provider", "选择提供者")}</option>{providers.map((item) => <option key={item.key} value={item.key}>{item.key}{item.search_supported ? " · search" : ""}</option>)}</select></label>
        <label className="lc-field">{tr("Supported URL", "支持的 URL")}<input type="url" value={url} onChange={(event) => { setUrl(event.target.value); setSelectedSource(null); }} placeholder="https://…" /></label>
      </div>
      {providers.find((item) => item.key === provider)?.search_supported ? <div className="lc-actions"><label className="lc-field">{tr("Search", "搜索")}<input value={query} onChange={(event) => setQuery(event.target.value)} /></label><button type="button" className="lc-btn lc-btn-secondary lc-btn-sm" disabled={pending || !query.trim()} onClick={() => void search()}>{tr("Search", "搜索")}</button></div> : <p role="status">{tr("Browse is unavailable; paste a supported URL.", "浏览不可用；请粘贴支持的 URL。")}</p>}
      {results.length ? <ul className="lc-presentation-search-results">{results.map((item, index) => <li key={`${String(item.title)}-${index}`}><button type="button" className="lc-btn lc-btn-outline lc-btn-sm" onClick={() => { setSelectedSource(item); setUrl(""); }}>{String(item.title || item.provider || tr("Source", "来源"))}</button></li>)}</ul> : null}
    </>}
    <div className="lc-actions lc-presentation-source-actions"><button type="button" className="lc-btn lc-btn-secondary lc-btn-sm" disabled={pending || !stepKey} onClick={() => void attach()}>{tr("Attach cue", "添加提示点")}</button></div>
    {cues.length ? <ul className="lc-presentation-cue-list" aria-label={tr("Attached cues", "已添加提示点")}>{cues.map((cue) => <li key={cue.id}><span>{String(cue.source.title || cue.source.type || tr("Source", "来源"))} · {steps.find((step) => step.key === cue.step_key)?.title || cue.step_key} · {Number(cue.source.slide_index ?? 0) + 1}</span><span className="lc-actions">{cue.valid === false ? <strong role="status">{cue.status === "reattach_required" ? tr("Reattach required", "需要重新关联") : tr("Unavailable", "不可用")}</strong> : <button type="button" className="lc-btn lc-btn-secondary lc-btn-sm" disabled={pending} onClick={() => void launch(cue)}>{tr("Launch", "启动")}</button>}<button type="button" className="lc-btn lc-btn-danger lc-btn-sm" disabled={pending} onClick={() => void remove(cue)}>{tr("Remove", "移除")}</button></span></li>)}</ul> : <p>{tr("No cues attached yet.", "尚未添加提示点。")}</p>}
    {status ? <p role="status" className={`lc-builder-status-${statusTone}`}>{status}</p> : null}
  </section>;
}

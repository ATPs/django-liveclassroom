...
function StudentViewControls({
  ...
  useEffect(() => {
    getJson<{ participants: Participant[] }>(participantsUrl)
      ...
      .catch((error: unknown) => setStatus(error instanceof Error ? error.message : "Unable to load participants."));
  }, [participantsUrl]);
  ...
  const inspectSelection = (token: string, active = false) => {
    ...
    setStatus(active ? "Acting as selected participant." : "Inspecting selected participant.");
  };
      ...
      <button
        ...
        onClick={() => {
          ...
          void postJson<{ act_as_token: string }>(activateUrl, { participant_id: participantId, confirm: true })
            ...
            .catch((error: unknown) => setStatus(error instanceof Error ? error.message : "Unable to activate act-as."));
        }}
      >
        Act as participant
      </button>

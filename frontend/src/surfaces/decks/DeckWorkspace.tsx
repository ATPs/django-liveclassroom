...
function DeckWorkspace({ apiRoot, assetsUrl, previewTemplate, deckUrlTemplate, libraryUrl, initialDeckId = "" }: { apiRoot: string; assetsUrl: string; previewTemplate: string; deckUrlTemplate: string; libraryUrl: string; initialDeckId?: string }) {
  ...
  return <div className="lc-deck-root">
    ...
    {deck ? <nav className="lc-editor-view-tabs" aria-label={tr("Deck editor regions", "幻灯片编辑区域")}>
      <button type="button" aria-pressed={editorView === "outline"} onClick={() => setEditorView("outline")}>{tr("Outline", "目录")}</button>
      <button type="button" aria-pressed={editorView === "edit"} onClick={() => setEditorView("edit")}>{tr("Edit", "编辑")}</button>
      <button type="button" aria-pressed={editorView === "preview"} onClick={() => setEditorView("preview")}>{tr("Preview", "预览")}</button>
    </nav> : null}
    <div className="lc-deck-layout" data-editor-view={editorView}>
      <aside className="lc-deck-list"><h3>{tr("Your decks", "我的幻灯片")}</h3>{!decks.length && <p>{tr("Create your first deck.", "创建第一份幻灯片。")}</p>}{decks.map((item) => <button type="button" key={item.id} className={`lc-deck-list-item ${deck?.id === item.id ? "lc-deck-selected" : ""}`} onClick={() => requestNavigation(() => { setEditorView("edit"); selectDeck(item); })}>{item.title}</button>)}</aside>
      {deck && <section className="lc-deck-editor">
        <div className="lc-deck-toolbar"><label>{tr("Title", "标题")}<input value={deck.title} onChange={(event) => edit({ title: event.target.value })} /></label><label>{tr("Theme", "主题")}<select value={["default", "light", "dark"].includes(deck.theme) ? deck.theme : "default"} onChange={(event) => edit({ theme: event.target.value })}><option value="default">{tr("Default (light)", "默认（浅色）")}</option><option value="light">{tr("Light", "浅色")}</option><option value="dark">{tr("Dark", "深色")}</option></select></label><div className="lc-actions"><button type="button" className="lc-btn lc-btn-primary" onClick={() => void save()} disabled={saving}>{tr("Save", "保存")}</button>{deck.id > 0 && <><button type="button" className="lc-btn lc-btn-outline" onClick={copy}>{tr("Copy", "复制")}</button><button type="button" className="lc-btn lc-btn-outline" onClick={exportDeck}>{tr("Export Markdown", "导出 Markdown")}</button><label className="lc-deck-include-notes"><input type="checkbox" checked={includeNotes} onChange={(event) => setIncludeNotes(event.target.checked)} /> {tr("Include private notes", "包含私密备注")}</label><button type="button" className="lc-btn lc-btn-danger" onClick={remove}>{tr("Delete", "删除")}</button></>}</div></div>
        <div className="lc-deck-edit-layout"><section className="lc-deck-slides"><div className="lc-actions"><h3>{tr("Slides", "页面")}</h3><button type="button" className="lc-btn lc-btn-sm lc-btn-primary" onClick={() => { const slide = { ...newSlide(), position: deck.slides.length + 1 }; edit({ slides: [...deck.slides, slide] }); setSelected(deck.slides.length); updateQuery({ slide: slide.key }, { guard: false }); }}>{tr("Add slide", "添加页面")}</button></div>{deck.slides.map((slide, index) => <button type="button" key={slide.key} className={`lc-deck-slide-item ${index === selected ? "lc-deck-selected" : ""}`} onClick={() => { setEditorView("edit"); selectSlide(index); }}>{index + 1}. {slide.markdown.split("\n")[0].replace(/^#+\s*/, "") || tr("Slide", "页面")}</button>)}</section>
          {currentSlide && <section className="lc-deck-slide-form"><div className="lc-actions"><button type="button" className="lc-btn lc-btn-sm lc-btn-outline" onClick={() => move(-1)} disabled={selected === 0}>{tr("Move up", "上移")}</button><button type="button" className="lc-btn lc-btn-sm lc-btn-outline" onClick={() => move(1)} disabled={selected === deck.slides.length - 1}>{tr("Move down", "下移")}</button><button type="button" className="lc-btn lc-btn-sm lc-btn-danger" onClick={() => { const slides = deck.slides.filter((_, index) => index !== selected); edit({ slides }); updateQuery({ slide: slides[Math.max(0, selected - 1)]?.key }, { guard: false }); }} disabled={deck.slides.length === 1}>{tr("Delete slide", "删除页面")}</button></div><label>{tr("Markdown", "Markdown")}<textarea aria-label={tr("Markdown", "Markdown")} rows={13} value={currentSlide.markdown} onChange={(event) => editSlide({ markdown: event.target.value })} /></label><label>{tr("Private presenter notes", "仅演讲者可见的备注")}<textarea aria-label={tr("Private presenter notes", "仅演讲者可见的备注")} rows={4} value={currentSlide.notes ?? ""} onChange={(event) => editSlide({ notes: event.target.value })} /></label><fieldset><legend>{tr("Attached files", "附加文件")}</legend>{!assets.length && <p>{tr("Upload a reusable file from a lesson first.", "请先从教案上传可复用文件。")}</p>}{assets.map((asset) => <label key={asset.id}><input type="checkbox" checked={currentSlide.asset_ids.includes(asset.id)} onChange={(event) => editSlide({ asset_ids: event.target.checked ? [...currentSlide.asset_ids, asset.id] : currentSlide.asset_ids.filter((id) => id !== asset.id) })} /> {asset.name}</label>)}</fieldset></section>}</div>
    ...
    <section className="lc-card lc-deck-portability" aria-label={tr("Markdown import", "Markdown 导入")}>
      ...
      <div className="lc-actions"><button type="button" className="lc-btn lc-btn-outline" onClick={previewImport} disabled={!importText.trim()}>{tr("Preview import", "预览导入")}</button><button type="button" className="lc-btn lc-btn-primary" onClick={commitImport} disabled={!importPreview?.valid}>{tr("Create imported deck", "创建导入幻灯片")}</button></div>

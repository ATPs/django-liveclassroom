...
function LearningWorkspace(props: Props) {
  ...
  return <div className="lc-learning-root lc-wide">
    ...
    {error ? <div role="alert"><p className="lc-form-error">{error}</p><button type="button" className="lc-btn lc-btn-outline" onClick={() => setRetry(value => value + 1)}>{tr("Retry", "重试")}</button></div> : null}
    ...
    <div className="lc-actions lc-browse-toolbar"><label className="lc-field"><span>{tr("Search", "搜索")}</span><input value={search} onChange={event => { typing.current = true; setSearch(event.target.value); }} /></label><label className="lc-field"><span>{tr("Sort", "排序")}</span><select value={sort} onChange={event => updateQuery({ sort: event.target.value, page: null }, { replace: true })}><option value="title">{tr("Title", "标题")}</option><option value="-title">{tr("Title descending", "标题降序")}</option><option value="recent">{tr("Recently updated", "最近更新")}</option></select></label></div>
    {loading ? <p role="status">{tr("Loading…", "正在加载…")}</p> : rows ? <section className="lc-learning-section"><h2>{label(tab)} <span className="lc-workspace-meta">({rows.count})</span></h2>{rows.items.length ? <ul>{rows.items.map(row => <li key={row.id ?? row.public_id ?? row.url}><NavigationLink href={preserveLocale(row.url)}>{row.title}</NavigationLink>{row.status ? <span className="lc-status-chip">{row.status}</span> : null}</li>)}</ul> : <p className="lc-empty-copy">{tr("No available items match this view.", "此视图中没有可用的内容。")}</p>}<nav className="lc-actions lc-results-pager" aria-label={tr("Pagination", "分页")}><button type="button" className="lc-btn lc-btn-outline" disabled={!rows.previous} onClick={() => turnPage(rows.previous)}>{tr("Previous", "上一页")}</button><span>{tr("Page", "页")} {rows.page}</span><button type="button" className="lc-btn lc-btn-outline" disabled={!rows.next} onClick={() => turnPage(rows.next)}>{tr("Next", "下一页")}</button></nav></section> : null}</>}
  </div>;
}

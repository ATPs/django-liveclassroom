...
function JoinPrompt({ onJoined }: { onJoined: (name: string) => void }) {
  const t = useT();
  ...
  const joinByName = useCallback(
    async (name: string) => {
      ...
      try {
        ...
        setJoinError(error instanceof Error ? error.message : t("unavailable"));
      }
    },
    [bootstrap, t],
  );
    ...
    <>
      ...
      <div id="student-content" data-liveclassroom-content>
        {joined ? (
          ...
          <JoinPrompt onJoined={(name) => void joinByName(name)} />
        ) : signInRequired ? (

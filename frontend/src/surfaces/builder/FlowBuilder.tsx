  ...
  return (
    <div className="lc-builder-step-preview">
      ...
      {choiceKinds.includes(typeKey) ? (
        ...
        <input type="text" className="lc-preview-input" placeholder={typeKey === "liveclassroom.word_cloud" ? "Enter a word…" : "Enter your answer…"} disabled />
...
function AddStepForm({
  ...
  const submit = async () => {
    ...
    if (isChoice) {
      ...
      payload = {
        kind: "activity",
        title: title || `Timer ${dur}s`,
        activity_definition: { title: title || `Timer ${dur}s`, type_key: type, definition: { duration_seconds: dur, label: title || "Timer" } },
      };
      ...
      payload = { kind: "markdown", title: title || "Lecture Note", content: { markdown: md } };
    } else {
      ...
      payload = {
        kind: "activity",
        title: title || "Media Presentation",
        activity_definition: {
          title: title || "Media Presentation",
          type_key: type,
...
function FlowBuilder({ flowsUrl, sessionId, initialFlowId }: { flowsUrl: string; sessionId: number | null; initialFlowId: number | null }) {
  const t = useT();
  ...
  const createFlow = async () => {
    const title = window.prompt(t("newFlowPrompt"), "New Lesson Flow");
  ...
  const duplicateFlow = async () => {
    ...
    const title = window.prompt(t("duplicateFlowPrompt"), `Copy of ${currentFlow.title}`);
  ...
  const saveSessionAsFlow = async () => {
    if (!sessionId) return;
    const title = window.prompt(t("saveAsFlowPrompt"), "Classroom Flow");
    if (!title || !title.trim()) return;
    try {
...
export function mountBuilder(container: HTMLElement): void {
  ...
  root.render(
    <LocaleProvider initial={locale} root={container}>
      <FlowBuilder flowsUrl={flowsUrl} sessionId={sessionId} initialFlowId={flowId} />
    </LocaleProvider>,
  );
}

...
function AssessmentQuestionRow({
  ...
  return (
    <li className={`lc-card lc-assessment-item${selected ? " lc-assessment-selected" : ""}`} data-assessment-item={item.key}>
      <div className="lc-assessment-item-heading">
        <button type="button" className="lc-assessment-item-number" aria-label={`${tr(locale, "Select question", "选择题目")} ${index + 1}`} aria-current={selected ? "true" : undefined} onClick={() => onSelect(item.key)}>{index + 1}</button>
...
function AssessmentBuilder({ apiRoot, initialAssessmentId = "", assessmentUrlTemplate, libraryUrl }: AssessmentBuilderProps & { initialAssessmentId?: string; assessmentUrlTemplate: string; libraryUrl: string }) {
  ...
  const savedDraft = React.useRef<Assessment | null>(null);
  const openGeneration = React.useRef(0);
  // The collection route is also the teacher's grading entry point. Keep that
  // useful queue visible there, while a concrete assessment still opens in its
  // editor by default.
  const [panelSelection, selectPanelUrl] = useQuerySelection("panel", initialAssessmentId ? "editor" : "results");
  const activePanel = ["editor", "settings", "picker", "preview", "results"].includes(panelSelection) ? panelSelection : "editor";
  const showPicker = activePanel === "picker";
  const showPreview = activePanel === "preview";
  const setShowPicker = (show: boolean) => { if (show || panelSelection === "picker") selectPanelUrl(show ? "picker" : "editor", { replace: true }); };
  const setShowPreview = (show: boolean) => { if (show || panelSelection === "preview") selectPanelUrl(show ? "preview" : "editor", { replace: true }); };
  useEffect(() => {
    if (!["editor", "settings", "picker", "preview", "results"].includes(panelSelection)) selectPanelUrl("editor", { replace: true });
  }, [panelSelection, selectPanelUrl]);
  ...
  const presetSaving = false;
  const [publishing, setPublishing] = useState(false);
  const [assessmentSelection] = useQuerySelection("assessment");
  const [itemSelection, selectItemUrl] = useQuerySelection("item");
  const locationPath = useLocationPath();
  useNavigationHeading("lc-assessment-heading");
  const routeAssessmentId = useMemo(() => {
    const match = new URL(locationPath, window.location.href).pathname.match(/\/assessments\/(\d+)\/?$/);
  ...
  const open = async (assessment: Assessment, updateUrl = true) => {
    ...
    try {
      ...
      const loaded = { ...detail, items: normalizeItems(detail.items ?? []), sections: sectionData.sections ?? [] };
      savedDraft.current = loaded;
      setDraft(loaded);
      setSections(sectionData.sections ?? []);
      setItemsDirty(false);
      if (updateUrl) updateLocation(routeUrl(assessmentUrlTemplate.replace(/\/0\//, `/${assessment.id}/`), { assessment: null, item: null }), { focusId: "lc-assessment-heading" });
      else if (!routeAssessmentId && assessmentSelection) updateLocation(routeUrl(assessmentUrlTemplate.replace(/\/0\//, `/${assessment.id}/`), { item: itemSelection || null, panel: panelSelection === "editor" ? null : panelSelection }), { replace: true });
    } catch (reason) { setError(reason instanceof Error ? reason.message : text("Unable to open assessment.", "无法打开测验。")); }
  };

  useEffect(() => {
    const selectedId = routeAssessmentId ?? (assessmentSelection ? Number(assessmentSelection) : null);
    if (!selectedId || draft?.id === selectedId) return;
    const selected = assessments.find((item) => item.id === selectedId);
    if (selected) void open(selected, false);
  }, [assessmentSelection, assessments, draft?.id, routeAssessmentId]); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (!draft || !itemSelection) return;
    if (routeAssessmentId && draft.id !== routeAssessmentId) return;
    if (!draft.items.some((item) => item.key === itemSelection)) selectItemUrl(draft.items[0]?.key ?? "", { replace: true });
  }, [draft, itemSelection, routeAssessmentId, selectItemUrl]);

  useUnsavedChangesWarning(Boolean(draft && (itemsDirty || !draft.id)));

  const create = () => {
    savedDraft.current = null;
    openGeneration.current += 1;
    updateLocation(routeUrl(libraryUrl), { guard: false });
    ...
    setSections([]); setItemsDirty(true); selectPanelUrl("editor", { replace: true }); setError(""); setNotice(""); setConflict(false); selectItemUrl("", { replace: true });
  };
  ...
  const updateSetting = (key: string, value: unknown) => {
    if (!draft) return;
    setDraft({ ...draft, settings: { ...draft.settings, [key]: value } }); setItemsDirty(true);
  };
  ...
  const saveSections = async () => {
    ...
    try {
      ...
      setDraft((current) => current ? { ...current, version: saved.version } : current);
      if (savedDraft.current) savedDraft.current = { ...savedDraft.current, version: saved.version, sections: saved.sections ?? sections };
  ...
  const addPool = () => {
    ...
    setSections([{ ...first, entries: [...first.entries, { key: globalThis.crypto?.randomUUID?.() ?? `pool-${Date.now()}`, kind: "pool", bank_id: banks[0].id, sample_size: 1, points: "1", shuffle_options: false }] }, ...sections.slice(1)]); setItemsDirty(true);
  };
  ...
  const publish = async () => {
    ...
    try {
      ...
      selectPanelUrl("results", { guard: false });
    } catch (reason) {
  ...
  const addQuestion = async (definitionId: number, revisionId?: number | null, question?: QuestionPickerSelection) => {
    ...
    setItemsDirty(true); selectPanelUrl("picker", { replace: true, guard: false }); setNotice(text("Question added. Its current revision is pinned when you save.", "题目已添加。保存时会固定当前版本。"));
  };
  ...
  const saveDraft = async (): Promise<boolean> => {
    ...
    if (!draft.title.trim()) { setError(text("An assessment title is required.", "请填写测验标题。")); return false; }
    if (draft.items.some((item) => !/^\d+(?:\.\d{1,6})?$/.test(item.points.trim()) || Number(item.points) <= 0 || Number(item.points) > 1000)) {
      setError(text("Points must be positive numbers up to 1000 with at most six decimals.", "分值必须是大于零且不超过1000的数字，最多六位小数。")); return false;
    }
    ...
    try {
      ...
      if (!draft.id) {
        ...
        const itemValues = (items: AssessmentItem[]) => items.map(({ key, question_revision_id, points }) => ({ key, question_revision_id, points }));
        if (JSON.stringify(itemValues(draft.items)) !== JSON.stringify(itemValues(savedDraft.current?.items ?? []))) {
          saved = await putJson<Assessment>(endpoint(apiRoot, `assessments/${draft.id}/items/`), {
            expected_version: saved.version,
            items: draft.items.map(({ key, question_revision_id, points }) => ({ key, revision_id: question_revision_id, points })),
          }, requestKey("items"));
          acknowledgedVersion = saved.version;
          // The item replacement endpoint also resets the server's section
          // plan to its compatibility default. Keep that acknowledged server
          // baseline so a later section-save failure can still be discarded
          // without pretending the old custom plan was persisted.
          savedDraft.current = {
            ...saved,
            items: normalizeItems(saved.items ?? draft.items),
      ...
      setDraft({ ...saved, items: normalizeItems(saved.items ?? []) }); setSections(saved.sections ?? sections); setItemsDirty(false); setAssessments((previous) => [saved, ...previous.filter((item) => item.id !== saved.id)]); updateLocation(routeUrl(assessmentUrlTemplate.replace(/\/0\//, `/${saved.id}/`), { assessment: null, item: itemSelection || null, panel: activePanel === "editor" ? null : activePanel }), { replace: true, focusId: "lc-assessment-heading" }); setNotice(text("Assessment saved.", "测验已保存。"));
      return true;
    } catch (reason) {
      if (acknowledgedVersion !== null) {
        setDraft((current) => current ? { ...current, version: acknowledgedVersion ?? current.version } : current);
        setItemsDirty(true);
  ...
  const save = (event: React.FormEvent<HTMLFormElement>) => { event.preventDefault(); void saveDraft(); };
  const { requestNavigation, dialog: unsavedDialog } = useUnsavedNavigationGuard({
    dirty: Boolean(draft && (itemsDirty || !draft.id)), onSave: saveDraft,
    onBeforeLeave: () => { setDraft(savedDraft.current); setSections(savedDraft.current?.sections ?? []); setItemsDirty(false); }, labels: {
      title: text("Unsaved assessment changes", "未保存的测验更改"),
      body: text("Save this assessment before opening another one?", "打开另一份测验前保存当前更改？"),
      save: text("Save and open", "保存并打开"), discard: text("Discard and open", "放弃并打开"), stay: text("Stay", "留在此处"),
    },
  });
  ...
  const copy = async () => {
    ...
    try {
      ...
      savedDraft.current = { ...copied, items: normalizeItems(copied.items ?? []) };
      setDraft({ ...copied, items: normalizeItems(copied.items ?? []) }); setItemsDirty(false); setAssessments((previous) => [copied, ...previous.filter((item) => item.id !== copied.id)]); updateLocation(routeUrl(assessmentUrlTemplate.replace(/\/0\//, `/${copied.id}/`), { assessment: null, item: null }), { focusId: "lc-assessment-heading", guard: false }); setNotice(text("Assessment copied. The source remains unchanged.", "测验已复制，来源保持不变。")); void load().catch(() => undefined);
  ...
  const switchPanel = (panel: string) => selectPanelUrl(panel, { guard: false });
  const movePanelFocus = (event: React.KeyboardEvent<HTMLButtonElement>) => {
  ...
  return (
    <div className="lc-assessment-root">
      ...
      <header className="lc-builder-topbar">
        ...
        <button type="button" className="lc-btn lc-btn-primary" onClick={() => requestNavigation(create)}>{text("New assessment", "新建测验")}</button>
      </header>
      ...
      <div className="lc-assessment-layout">
        <aside className="lc-card lc-assessment-list" aria-label={text("Assessments", "测验列表")}>
          ...
          {draft?.items.length ? <nav className="lc-assessment-outline" aria-label={text("Question outline", "题目大纲")}><h2>{text("Questions", "题目")}</h2><ol>{draft.items.map((item, index) => <li key={item.key}><button type="button" className="lc-assessment-list-item" aria-current={itemSelection === item.key ? "true" : undefined} onClick={() => { selectItemUrl(item.key, { replace: true, guard: false }); switchPanel("picker"); }}>{index + 1}. {currentQuestions[item.definition_id]?.title || promptFor(item).slice(0, 65) || text("Question", "题目")}</button></li>)}</ol></nav> : null}
        </aside>
        <section className="lc-assessment-main">
          {!draft ? <section className="lc-card lc-assessment-empty"><h2>{text("Create a reusable assessment", "创建可复用测验")}</h2><p>{text("Choose New assessment, then add questions from a bank.", "选择“新建测验”，然后从题库添加题目。")}</p></section> : (
            <>
              <nav className="lc-assessment-tabs" role="tablist" aria-label={text("Assessment workspace", "测验工作区")}>
                {[
                  ["editor", text("Editor", "编辑")],
                  ["picker", text("Questions", "题目")],
                  ["preview", text("Preview", "预览")],
                  ["settings", text("Settings", "设置")],
                  ["results", text("Results", "结果")],
                ].map(([panel, label]) => <button key={panel} type="button" role="tab" id={`assessment-tab-${panel}`} aria-selected={activePanel === panel} aria-controls={`assessment-panel-${panel}`} tabIndex={activePanel === panel ? 0 : -1} disabled={panel === "preview" && !draft.items.length} onClick={() => switchPanel(panel)} onKeyDown={movePanelFocus}>{label}</button>)}
              </nav>
              <form className="lc-card lc-assessment-form" onSubmit={save} onChangeCapture={() => setItemsDirty(true)} hidden={activePanel !== "editor" && activePanel !== "settings"}>
                <section id="assessment-panel-editor" role="tabpanel" aria-labelledby="assessment-tab-editor" hidden={activePanel !== "editor"} className="lc-assessment-editor-panel">
                  <div className="lc-assessment-form-heading"><div><label htmlFor="assessment-title">{text("Title", "标题")}</label><input id="assessment-title" className="lc-input" value={draft.title} onChange={(event) => { setDraft({ ...draft, title: event.target.value }); setItemsDirty(true); }} maxLength={200} required /></div><p className="lc-workspace-meta">{itemsDirty ? text("Unsaved changes", "有未保存的更改") : text("Saved draft", "已保存草稿")} · v{draft.version}</p></div>
                  ...
                  <label htmlFor="assessment-instructions">{text("Instructions", "说明")}</label><textarea id="assessment-instructions" className="lc-textarea" rows={4} maxLength={20000} value={draft.instructions} onChange={(event) => { setDraft({ ...draft, instructions: event.target.value }); setItemsDirty(true); }} />
                  <div className="lc-actions"><button type="submit" className="lc-btn-primary" disabled={saving || presetSaving}>{saving ? text("Saving…", "保存中…") : text("Save draft", "保存草稿")}</button><button type="button" className="lc-btn-primary" onClick={() => void publish()} disabled={saving || publishing || !draft.items.length}>{publishing ? text("Publishing…", "发布中…") : text("Publish", "发布")}</button>{draft.id ? <button type="button" className="lc-btn-outline" onClick={() => void copy()} disabled={saving}>{text("Copy", "复制")}</button> : null}</div>
                ...
                <section id="assessment-panel-settings" role="tabpanel" aria-labelledby="assessment-tab-settings" hidden={activePanel !== "settings"} className="lc-assessment-settings-panel">
                  <h2>{text("Settings", "设置")}</h2>
                  <div className="lc-assessment-settings-grid">
                  ...
                  <label htmlFor="assessment-course">{text("Class (optional)", "班级（可选）")}</label><select id="assessment-course" className="lc-select" value={draft.course_id ?? ""} onChange={(event) => { setDraft({ ...draft, course_id: event.target.value ? Number(event.target.value) : null }); setItemsDirty(true); }}><option value="">{text("No class", "无班级")}</option>{courses.map((course) => <option key={course.id} value={String(course.id)}>{course.title}</option>)}</select>
                {draft.id ? <section className="lc-assessment-settings lc-assessment-question-plan">
                  <h3>{text("Question order and random pools", "题目顺序与随机题池")}</h3>
                  ...
                  {sections.map((section, sectionIndex) => <section key={section.key} className="lc-card">
                    <label>{text("Section title", "分组标题")}<input className="lc-input" value={section.title} onChange={(event) => { setSections(sections.map((row, index) => index === sectionIndex ? { ...row, title: event.target.value } : row)); setItemsDirty(true); }} /></label>
                    <label><input type="checkbox" checked={section.shuffle_questions} onChange={(event) => { setSections(sections.map((row, index) => index === sectionIndex ? { ...row, shuffle_questions: event.target.checked } : row)); setItemsDirty(true); }} /> {text("Randomize question order in this section", "随机排列本分组题目")}</label>
                    <ul>{section.entries.map((entry, entryIndex) => <li key={entry.key}>{entry.kind === "fixed" ? <label><input type="checkbox" checked={Boolean(entry.shuffle_options)} onChange={(event) => { setSections(sections.map((row, index) => index !== sectionIndex ? row : { ...row, entries: row.entries.map((candidate, candidateIndex) => candidateIndex === entryIndex ? { ...candidate, shuffle_options: event.target.checked } : candidate) })); setItemsDirty(true); }} /> {text("Shuffle options for fixed question", "随机排列固定题的选项")}</label> : <span>{text("Random pool", "随机题池")}: <select className="lc-select" value={entry.bank_id ?? ""} onChange={(event) => { setSections(sections.map((row, index) => index !== sectionIndex ? row : { ...row, entries: row.entries.map((candidate, candidateIndex) => candidateIndex === entryIndex ? { ...candidate, bank_id: Number(event.target.value) } : candidate) })); setItemsDirty(true); }}>{banks.map((bank) => <option key={bank.id} value={bank.id}>{bank.title}</option>)}</select> <label>{text("Questions", "题数")} <input className="lc-input" type="number" min="1" value={entry.sample_size ?? 1} onChange={(event) => { setSections(sections.map((row, index) => index !== sectionIndex ? row : { ...row, entries: row.entries.map((candidate, candidateIndex) => candidateIndex === entryIndex ? { ...candidate, sample_size: Number(event.target.value) } : candidate) })); setItemsDirty(true); }} /></label> <label>{text("Points each", "每题分值")} <input className="lc-input" inputMode="decimal" value={entry.points ?? ""} onChange={(event) => { setSections(sections.map((row, index) => index !== sectionIndex ? row : { ...row, entries: row.entries.map((candidate, candidateIndex) => candidateIndex === entryIndex ? { ...candidate, points: event.target.value } : candidate) })); setItemsDirty(true); }} /></label> <button type="button" className="lc-btn-sm lc-btn-danger" onClick={() => { setSections(sections.map((row, index) => index !== sectionIndex ? row : { ...row, entries: row.entries.filter((_, candidateIndex) => candidateIndex !== entryIndex) })); setItemsDirty(true); }}>{text("Remove pool", "移除题池")}</button></span>}</li>)}</ul>
                  </section>)}
                  ...
                  <div className="lc-actions"><button type="submit" className="lc-btn-primary" disabled={saving || presetSaving}>{saving ? text("Saving…", "保存中…") : text("Save draft", "保存草稿")}</button><button type="button" className="lc-btn-primary" onClick={() => void publish()} disabled={saving || publishing || !draft.items.length}>{publishing ? text("Publishing…", "发布中…") : text("Publish", "发布")}</button></div>
                </section>
              </form>
              <section id="assessment-panel-picker" role="tabpanel" aria-labelledby="assessment-tab-picker" className="lc-card lc-assessment-picker" hidden={!showPicker}><h2>{text("Questions", "题目")}</h2><p>{text("Select a question to pin its current revision, then adjust its order and points below.", "选择题目即可固定当前版本，然后在下方调整顺序和分值。")}</p><QuestionBankWorkspace apiRoot={apiRoot} picker onPick={addQuestion} pickerLabel={{ en: "Add to assessment", zh: "添加到测验" }} /><section className="lc-assessment-items" aria-labelledby="assessment-items-heading"><div className="lc-assessment-section-heading"><h2 id="assessment-items-heading">{text("Selected questions", "已选题目")}</h2><span>{draft.items.length}</span></div>{draft.items.length ? <ol>{draft.items.map((item, index) => (itemSelection ? item.key === itemSelection : index === 0) ? <AssessmentQuestionRow key={item.key} item={item} index={index} itemCount={draft.items.length} apiRoot={apiRoot} locale={locale} current={currentQuestions[item.definition_id]} onMove={move} onPoints={points} onRemove={remove} onReplace={replace} selected={itemSelection === item.key} onSelect={(key) => selectItemUrl(key, { guard: false })} /> : null)}</ol> : <div className="lc-card lc-empty-notice"><p>{text("No questions selected yet. Use the question bank above to add one.", "还没有选择题目。请在上方题库中添加题目。")}</p></div>}</section></section>
              <section id="assessment-panel-preview" role="tabpanel" aria-labelledby="assessment-tab-preview" className="lc-assessment-preview" hidden={!showPreview}><h2 id="assessment-preview-heading">{text("Teacher preview", "教师预览")}</h2><p className="lc-guidance-risk">{text("This private preview uses the exact pinned revisions and includes answer keys only for you.", "此私有预览使用固定版本，仅向你显示答案。")}</p>{draft.items.map((item) => <QuestionPinnedPreview key={item.key} item={item} apiRoot={apiRoot} locale={locale} />)}</section>
            </>
          )}
        </section>
      </div>
      {activePanel === "results" ? <section id="assessment-panel-results" role="tabpanel" aria-label={text("Assessment results", "测验结果")} className="lc-assessment-results-panel"><ManualGradingQueue apiRoot={apiRoot} /><AssessmentResults apiRoot={apiRoot} assessmentId={draft?.id ?? null} /></section> : null}
...
export function mountAssessmentBuilder(el: HTMLElement): void {
  ...
  const libraryUrl = el.dataset.libraryUrl;
  if (!rawApiRoot || !assessmentUrlTemplate || !libraryUrl) return;
  const apiRoot = normalizeApiRoot(rawApiRoot);
  const locale = el.dataset.locale?.startsWith("zh") ? "zh-Hans" : "en";
  const root = createRoot(el);
  root.render(<LocaleProvider initial={locale} root={el}><AssessmentBuilder apiRoot={apiRoot} initialAssessmentId={el.dataset.assessmentId || ""} assessmentUrlTemplate={assessmentUrlTemplate} libraryUrl={libraryUrl} /></LocaleProvider>);

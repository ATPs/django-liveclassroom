...
function Pager({ page, name }: { page: Page<unknown>; name: string }) {
  ...
  return <nav className="lc-actions lc-results-pager" aria-label={zh ? "分页" : "Pagination"}>
...
function Submission({ url, runId, itemKey, panel, onGradeSaved }: { url: string; runId: string; itemKey: string; panel: string; onGradeSaved: () => void }) {
  ...
  return <aside className="lc-results-submission" aria-label={zh ? "提交详情" : "Submission detail"}>
    <header className="lc-results-submission-heading"><div><p className="lc-kicker">{zh ? "提交详情" : "Submission detail"}</p><h2>{data.student_identifier} · #{data.attempt_number}</h2></div><p className="lc-workspace-meta">{data.status} · {data.earned_points ?? "—"}/{data.possible_points ?? "—"}</p></header>
    <label className="lc-field"><span>{zh ? "题目" : "Question"}</span><select className="lc-input" value={selected?.item_key ?? ""} onChange={event => updateQuery({ item: event.target.value })}>{items.map((item, index) => <option key={item.item_key} value={item.item_key}>{index + 1}</option>)}</select></label>
    ...
    {selected ? <>
      <section className="lc-results-prompt"><h3>{zh ? "题干" : "Prompt"}</h3><MarkdownView markdown={selected.prompt} /></section>
      {panel === "history" ? <ol className="lc-grade-history">{selected.grade_decisions.map((decision, index) => <li key={index}><p>{decision.created_at} · {decision.source} · {decision.awarded_points ?? "—"}</p><p>{decision.comment}</p><p>{decision.reason}</p></li>)}</ol> : <>
        <section className="lc-results-answer"><h3>{zh ? "学生答案" : "Student answer"}</h3><pre>{typeof selected.answer === "string" ? selected.answer : JSON.stringify(selected.answer, null, 2)}</pre></section>
        <section className="lc-results-grade"><h3>{zh ? "成绩与反馈" : "Grade and feedback"}</h3><p>{selected.current_grade?.awarded_points ?? "—"} / {selected.current_grade?.possible_points ?? "—"}</p>{selected.current_grade?.feedback ? <p>{selected.current_grade.feedback}</p> : <p className="lc-workspace-meta">{zh ? "尚无反馈。" : "No feedback yet."}</p>}</section>
...
function ResultsWorkspace({ apiRoot, runsUrl, summaryUrl, parentUrl, navigationUrl }: { apiRoot: string; runsUrl: string; summaryUrl?: string; parentUrl?: string; navigationUrl?: string }) {
  ...
  return <div className="lc-results-root">
    ...
    <header className="lc-results-header"><div><p className="lc-kicker">{zh ? "测验" : "Assessment"}</p><h1 id="lc-results-heading" tabIndex={-1}>{summary.data?.scope.title ?? (zh ? "结果与评分" : "Results and grading")}</h1></div>
      <div className="lc-actions"><button className="lc-btn lc-btn-outline" aria-pressed={panel === "grading"} onClick={() => updateQuery({ panel: panel === "grading" ? "answer" : "grading" })}>{zh ? "人工评分队列" : "Manual grading queue"}</button></div>
    </header>
    ...
    {summary.data ? <section className="lc-results-summary" aria-label={zh ? "评分概览" : "Grading summary"}><dl><div><dt>{zh ? "名册" : "Roster"}</dt><dd>{summary.data.roster.denominator}</dd></div><div><dt>{zh ? "已评分" : "Graded"}</dt><dd>{summary.data.overall.graded_count}</dd></div><div><dt>{zh ? "待处理" : "Pending"}</dt><dd>{summary.data.overall.pending_count}</dd></div></dl></section> : null}
    {contexts.data?.contexts.some(context => context.results_url) ? <section className="lc-results-contexts"><h2>{zh ? "班级和课程汇总" : "Class and course summaries"}</h2><div className="lc-actions">{contexts.data.contexts.filter(context => context.results_url).map(context => <NavigationLink key={`${context.kind}-${context.id}`} className="lc-btn lc-btn-outline lc-btn-sm" href={context.results_url!}>{context.title}</NavigationLink>)}</div></section> : null}
    {panel === "grading" ? <><div className="lc-actions lc-results-grading-actions"><button className="lc-btn lc-btn-outline" onClick={() => updateQuery({ run: null, attempt: null, item: null })}>{zh ? "所有待评分答案" : "All pending responses"}</button></div><ManualGradingQueue apiRoot={apiUrl(apiRoot, "")} runId={runId || undefined} attemptId={attemptId || undefined} itemKey={params.get("item") || undefined} classId={classId} courseId={courseId} /></> : <div className="lc-results-layout">
      <aside className="lc-results-filters"><h2>{zh ? "已发布测验" : "Published assessments"}</h2>
        <label className="lc-field"><span>{zh ? "搜索测验" : "Search assessments"}</span><input className="lc-input" value={search} onChange={event => { searchPending.current = true; setSearch(event.target.value); }} /></label>
        <label className="lc-field"><span>{zh ? "排序" : "Sort"}</span><select className="lc-input" value={params.get("sort") ?? "recent"} onChange={event => updateQuery({ sort: event.target.value, page: null }, { replace: true })}><option value="recent">{zh ? "最近" : "Recent"}</option><option value="title">{zh ? "标题" : "Title"}</option></select></label>
      ...
      <section className="lc-results-table-region"><header className="lc-results-table-heading"><h2>{attempts.data?.run.title ?? (zh ? "提交" : "Submissions")}</h2></header>
        {!runId ? <p>{zh ? "选择测验以查看提交。" : "Select an assessment to view submissions."}</p> : !attempts.data ? <ReadState error={attempts.error} retry={attempts.retry} /> : <>
          <div className="lc-results-table-toolbar"><label className="lc-field"><span>{zh ? "状态" : "Status"}</span><select className="lc-input" value={params.get("status") ?? ""} onChange={event => updateQuery({ status: event.target.value, attempt_page: null }, { replace: true })}><option value="">{zh ? "全部" : "All"}</option><option value="in_progress">{zh ? "进行中" : "In progress"}</option><option value="submitted">{zh ? "已提交" : "Submitted"}</option></select></label></div>
          <div className="lc-results-table-wrap"><table><thead><tr><th>{zh ? "学生" : "Student"}</th><th>{zh ? "次数" : "Attempt"}</th><th>{zh ? "状态" : "Status"}</th><th>{zh ? "成绩" : "Score"}</th></tr></thead><tbody>{attempts.data.items.map(attempt => <tr key={attempt.attempt_id} aria-selected={attemptId === attempt.attempt_id}><td>{attempt.detail_url ? <button className="lc-btn lc-btn-outline lc-btn-sm" onClick={() => updateQuery({ attempt: attempt.attempt_id, item: null })}>{attempt.student_identifier}</button> : attempt.student_identifier}</td><td>{attempt.attempt_number}</td><td><span className="lc-status-chip">{attempt.status}</span></td><td>{attempt.earned_points ?? "—"}/{attempt.possible_points ?? "—"}</td></tr>)}</tbody></table></div>

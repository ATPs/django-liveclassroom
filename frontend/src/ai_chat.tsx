...
function AiChat({ options, apiRoot }: { options: AiChatOptions; apiRoot: string }) {
  ...
  const selectThread = useCallback(async (threadId: number) => {
    ...
    try {
      ...
      setStatus(err instanceof Error ? err.message : "Failed to load thread");
    }
  }, []);
  ...
  const loadThreads = useCallback(async () => {
    try {
      ...
      if (loaded.length === 0) {
        const created = await postJson<AuthoringThread>(apiUrl("authoring/threads/"), { title: "Authoring Assistant" });
        loaded = [created];
      }
      setThreads(loaded);
      ...
      setStatus(err instanceof Error ? err.message : "Failed to load threads");
    }
    ...
    pollTimerRef.current = window.setTimeout(async () => {
      try {
        ...
        if (job.status === "succeeded") {
          ...
          if (activeThreadId === threadId) await selectThread(threadId);
        } else if (job.status === "failed") {
          setGenerating(false);
          setStatus(job.error_code ? `AI error: ${job.error_code}` : "Generation failed.");
          activeJobRef.current = null;
        } else {
        ...
        setGenerating(false);
        setStatus("AI job polling error");
        activeJobRef.current = null;
      }
    }, 1000);
  }, [activeThreadId, selectThread]);
  ...
  const send = async () => {
    ...
    if (!backend_key || !model_identifier) {
      setStatus("Please select an AI model");
      return;
    }
  ...
  const createThread = async () => {
    const title = window.prompt(t("newFlowPrompt", locale), "Lesson helper");
    ...
    try {
      ...
      setStatus(err instanceof Error ? err.message : "Failed to create conversation");
    }
  };
          ...
          messages.map((msg) => (
            <div key={msg.id} className={`lc-ai-message lc-ai-message-${msg.role}`}>
              <div className="lc-ai-msg-header">
                <strong>{msg.role === "assistant" ? "AI Assistant" : "Teacher"}</strong>
